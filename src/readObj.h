...
namespace objl
{
	...
	// 计算从一点到平面的投影
	Vector3 ProjectToPlane(const Vector3& point, const Vector3& planePoint, const Vector3& planeNormal) {
		Vector3 v = point - planePoint;
		float len = v.dot(planeNormal);
		return point - planeNormal * len;
	}
	...
	public:
		...
		// how to output the meshes
		int vtype;
		bool has_matlib = false;
		std::string path_matlib;
		...
		bool LoadFile(std::string Path)
		{
			...
			while (std::getline(file, curline))
			{
				...
				if (algorithm::firstToken(curline) == "f")
				{
					...
					Triangle Tri;
					GenVerticesFromRawOBJ(vVerts, Positions, TCoords, Normals, curline, Tri);
				...
				if (algorithm::firstToken(curline) == "usemtl")
				{
					...
					has_usemtl = true;
					mtl_name = algorithm::tail(curline);
		...
		bool DumpFile(std::string Path)
		{
			...
			if (vtype == 3 || vtype == 4)
			{
				...
				{
					file << "v " << Normal.X << " " << Normal.Y << " " << Normal.Z << "\n";
				}
			}
